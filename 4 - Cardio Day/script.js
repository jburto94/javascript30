...
const longestLiving = inventors.sort((a, b) => {
...
(Only works when used in developer tools on specified link)
*/

// 7. Sort people alphabetically by last name
const alphabetizedLastName = people.sort();
console.log(alphabetizedLastName)

// 8. Sum up the instances of each of these:
const data = ['car', 'car', 'truck', 'truck', 'bike', 'walk', 
'car', 'van', 'bike', 'walk', 'car', 'van', 'car', 'truck' ];

const vehicle = data.reduce((obj, item) => {
  if(!obj[item]) {
    obj[item] = 0;
  }
  obj[item]++;
